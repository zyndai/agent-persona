...
Vendored from zyndai_agent.a2a.auth (v0.6.0). MUST mirror the SDK and
the TS counterpart byte-for-byte on the signing input — that's the
...
The only difference from the SDK source is the import block — we pull
crypto helpers from our local agent.zynd_identity instead of the SDK's
zyndai_agent.ed25519_identity. The contract those helpers expose is
the same.
"""
